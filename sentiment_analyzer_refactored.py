...
def analyze_sentiment(text):
    ...
    Returns:
        str: The sentiment label (positive, negative, or neutral), or None
             if the request could not be completed.
    """
    url = "https://api.text-processing.com/api/sentiment/"
    payload = {"text": text}

    # Securely read the API key from the environment rather than
    # hardcoding it in source. Fail fast with a clear message if it
    # is missing, instead of sending an unauthenticated request.
    api_key = os.environ.get("TEXT_PROCESSING_API_KEY")
    if not api_key:
        print(
            "Configuration Error: TEXT_PROCESSING_API_KEY environment "
            "variable is not set.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        print(f"Connection Error: Could not connect to the API: {e}", file=sys.stderr)
        return None
